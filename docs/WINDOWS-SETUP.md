...
最新版のJavaScriptを公開場所へコピーし直します。

```powershell
cd C:\Apache24\htdocs\c-questionaire
powershell -ExecutionPolicy Bypass -File .\deploy\install-simple.ps1
Test-Path C:\Apache24\htdocs\c-questionaire\assets\app.js
```

`True`を確認し、ブラウザーで`Ctrl＋F5`を押します。「画面を準備しています」という赤いメッセージが消えればJavaScriptは動作しています。今回のJavaScriptは、電子カルテ端末で互換表示される場合も考慮し、新しい構文に依存しない実装です。
...
回答は次のSQLiteファイルへ保存されます。

```text
C:\Apache24\htdocs\c-questionaire\storage\questionnaire.sqlite
```

管理画面から出力するExcel／CSVは、このファイルに保存された回答を読み出しています。運用中はSQLiteファイルをExcelなどで直接開いたり、移動したりしないでください。
