...
if ($path === '/submit' && $method === 'POST') {
    ...
    foreach ($people as $i => $person) {
        ...
        foreach (['target_group','relationship','name','kana','birth_date','gender','patient_history','wants_second_dose','postal_code','address','phone','dose_no','appointment_date','appointment_time'] as $field) if (trim((string)($person[$field] ?? '')) === '') $errors[] = ($i + 1) . '人目の必須項目を入力してください。';
