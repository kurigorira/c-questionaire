...
<section class="hero"><div><p class="eyebrow">2026年度 職員・職員家族向け</p><h1>インフルエンザ<br><em>予防接種 事前アンケート</em></h1><p>職員本人とご家族を、1台のPCからまとめて申し込めます。</p></div><div class="hero-date"><small>回答期限</small><strong>{$deadline}</strong><span>自己負担金 {$fee}円 / 回</span></div></section>
...
if ($path === '/submit' && $method === 'POST') {
    ...
    $allowed = ['target_group'=>['高校生以上','小児（6か月～中学生）','2歳～小学生'],'relationship'=>['本人','配偶者','子','その他家族'],'gender'=>['男性','女性','回答しない'],'patient_history'=>['あり','なし','不明'],'wants_second_dose'=>['あり','なし'],'vaccine_method'=>['injection','nasal'],'dose_no'=>['1']];
    foreach ($people as $i => $person) {
        foreach (['target_group','relationship','name','kana','birth_date','gender','patient_history','wants_second_dose','vaccine_method','dose_no','appointment_date','appointment_time'] as $field) if (trim((string)($person[$field] ?? '')) === '') $errors[] = ($i + 1) . '人目の必須項目を入力してください。';
    ...
    foreach ($people as $p) $stmt->execute([$applicationId,$p['relationship'],$p['target_group'],$p['name'],$p['kana'],$p['birth_date'],$p['gender'],$p['patient_history'],$p['chart_no'] ?? '',$p['postal_code'] ?? '',$p['address'] ?? '',$p['phone'] ?? '',$p['vaccine_method'],1,$p['wants_second_dose'],$p['appointment_date'],$p['appointment_time'],$p['notes'] ?? '']);
    $pdo->commit();
    unset($_SESSION['csrf']);
