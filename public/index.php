...
function view(string $title, string $content, bool $admin = false): never {
    ...
    $nav = $admin ? '<a class="subtle" href="/admin">回答一覧</a><a class="subtle" href="/admin/logout">ログアウト</a>' : '<span class="badge">職員・ご家族専用</span>';
    echo "<!doctype html><html lang=\"ja\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>{$safeTitle}｜{$hospital}</title><link rel=\"stylesheet\" href=\"/assets/app.css\"></head><body><header><div><span class=\"hospital\">{$hospital}</span><span class=\"service\">インフルエンザ予防接種</span></div><nav>{$nav}</nav></header><main>{$content}</main><footer>個人情報は予防接種受付業務の目的にのみ利用します。</footer><script src=\"/assets/app.js\" defer></script></body></html>";
    exit;
}
...
if ($path === '/' && $method === 'GET') {
    ...
    if (!Eligibility::responsesOpen($config['deadline'])) {
        http_response_code(410);
        view('受付終了', "<section class=\"complete card\"><h1>受付は終了しました</h1><p>回答期限は {$deadline} でした。変更やご相談は病院担当者へお問い合わせください。</p></section>");
    }
    $fee = number_format($config['fee']);
    $childDates = implode('・', array_map(fn($d) => (new DateTimeImmutable($d))->format('n月j日'), $config['child_dates']));
    $regularRange = (new DateTimeImmutable($config['senior_start']))->format('n月j日') . '〜' . (new DateTimeImmutable($config['senior_end']))->format('n月j日');
    $regularTimes = implode(' / ', $config['regular_times']);
    $childTimes = implode(' / ', $config['child_times']);
    $season = (int)$config['season'];
    $scheduleConfig = Security::e(json_encode([
        'season' => $season,
        'seniorStart' => $config['senior_start'],
        'seniorEnd' => $config['senior_end'],
        'childDates' => $config['child_dates'],
        'regularTimes' => $config['regular_times'],
        'childTimes' => $config['child_times'],
    ], JSON_THROW_ON_ERROR));
    $token = Security::csrf();
    $content = <<<HTML
<section class="hero"><div><p class="eyebrow">{$season}年度 職員・職員家族向け</p><h1>インフルエンザ<br><em>予防接種 事前アンケート</em></h1><p>ご本人とご家族を、まとめてかんたんにお申し込みいただけます。</p></div><div class="hero-date"><small>回答期限</small><strong>{$deadline}</strong><span>自己負担金 {$fee}円 / 回</span></div></section>
<section class="notice"><strong>接種について</strong><div class="notice-grid"><p><b>高校生以上</b><br>{$regularRange}の平日<br>{$regularTimes}</p><p><b>6か月〜中学生</b><br>{$childDates}<br>{$childTimes}</p><p><b>経鼻ワクチン</b><br>2歳〜小学生・1回接種<br>注射も選択できます</p></div></section>
<form action="/submit" method="post" id="application-form" data-schedule-config="{$scheduleConfig}"><input type="hidden" name="csrf" value="{$token}">
...
if ($path === '/submit' && $method === 'POST') {
    if (!Eligibility::responsesOpen($config['deadline'])) { http_response_code(410); view('受付終了', '<section class="card"><h1>受付は終了しました</h1><p>回答期限を過ぎているため、送信できませんでした。</p><a class="button" href="/">戻る</a></section>'); }
    if (!Security::verifyCsrf($_POST['csrf'] ?? null)) { http_response_code(419); view('有効期限切れ', '<section class="card"><h1>画面の有効期限が切れました</h1><p>最初からやり直してください。</p><a class="button" href="/">戻る</a></section>'); }
    ...
    if ($errors) { $items = implode('', array_map(fn($e) => '<li>' . Security::e($e) . '</li>', array_unique($errors))); view('入力エラー', "<section class=\"card error\"><h1>入力内容をご確認ください</h1><ul>{$items}</ul><button type=\"button\" class=\"button\" data-history-back>入力画面に戻る</button></section>"); }
    ...
    view('受付完了', "<section class=\"complete card\"><div class=\"check\">✓</div><p class=\"eyebrow\">送信が完了しました</p><h1>お申し込みありがとうございます</h1><p>受付番号を控えてください。</p><div class=\"receipt\"><small>受付番号</small><strong>{$safe}</strong></div><p class=\"muted\">予約確定や変更については、病院担当者からの案内をご確認ください。</p><a class=\"button\" href=\"/\">トップへ戻る</a></section>");
}

if ($path === '/admin/login') {
    $credentials = Security::adminCredentials();
    if ($credentials === null) {
        http_response_code(503);
        view('管理画面は利用できません', '<section class="card error"><h1>管理画面は利用できません</h1><p>環境変数 ADMIN_USER と ADMIN_PASSWORD に初期値以外の認証情報を設定してください。</p></section>');
    }
...
if ($path === '/admin' || str_starts_with($path, '/admin/export')) {
    ...
    $trs = ''; foreach($rows as $r) { $methodName=$r['vaccine_method']==='nasal'?'経鼻':'注射'; $employeeContact=Security::e($r['employee_phone']) . ($r['employee_email'] !== '' ? '<br>' . Security::e($r['employee_email']) : ''); $trs.='<tr><td>'.Security::e($r['appointment_date']).'<br><small>'.Security::e($r['appointment_time']).'</small></td><td><b>'.Security::e($r['name']).'</b><br><small>'.Security::e($r['kana']).'</small></td><td>'.Security::e($r['employee_name']).'<br><small>'.Security::e($r['department']).'<br>'.$employeeContact.'</small></td><td><span class="pill">'.Security::e($methodName).'</span></td><td>'.Security::e($r['dose_no']).'回目</td><td>'.Security::e($r['status']).'</td></tr>'; }
    if(!$trs)$trs='<tr><td colspan="6" class="empty">まだ回答はありません。</td></tr>';
    $content="<section class=\"admin-head\"><div><p class=\"eyebrow\">ADMINISTRATION</p><h1>回答一覧</h1></div><div class=\"exports\"><a href=\"/admin/export.csv\">CSV</a><a class=\"button\" href=\"/admin/export.xlsx\">Excelをダウンロード</a></div></section><section class=\"stats\"><div><small>申込人数</small><strong>{$count}</strong></div><div><small>注射</small><strong>{$injection}</strong></div><div><small>経鼻ワクチン</small><strong>{$nasal}</strong></div></section><section class=\"table-card\"><table><thead><tr><th>接種日時</th><th>接種対象者</th><th>申込職員</th><th>方法</th><th>回数</th><th>状態</th></tr></thead><tbody>{$trs}</tbody></table></section>";
    view('回答一覧', $content, true);
}

http_response_code(404); view('ページが見つかりません', '<section class="card"><h1>ページが見つかりません</h1><a class="button" href="/">トップへ</a></section>');
